...
###############################################################################
# QUICK SETUP CHECKLIST GENERATOR
# Run this after Jenkins is installed to get your personalized setup checklist
###############################################################################

echo "=========================================="
echo "JENKINS CI/CD SETUP - PERSONALIZED CHECKLIST"
echo "=========================================="
echo ""
...
# Read Jenkins setup info if exists
if [ -f "/root/vito/jenkins-setup-info.txt" ]; then
    source <(grep "^JENKINS_PORT=" /root/vito/jenkins-setup-info.txt 2>/dev/null || echo "JENKINS_PORT=9002")
else
    JENKINS_PORT=9002
fi

cat > /root/vito/SETUP_CHECKLIST.md <<EOF
...
**Jenkins URL:** \`http://${SERVER_IP}:${JENKINS_PORT}\`
...
3. Configure:
   - **Payload URL:** \`http://${SERVER_IP}:${JENKINS_PORT}/github-webhook/\`
...
docker exec jenkins ssh -o StrictHostKeyChecking=no root@${SERVER_IP} "echo 'SSH connection successful'"
\`\`\`
...
sudo ufw allow from YOUR_IP_ADDRESS to any port ${JENKINS_PORT}
...
- [ ] Jenkins is accessible at \`http://${SERVER_IP}:${JENKINS_PORT}\`
...
- Verify firewall allows port ${JENKINS_PORT}
- Verify URL is correct: \`http://${SERVER_IP}:${JENKINS_PORT}/github-webhook/\`
...
echo "✅ Checklist generated: /root/vito/SETUP_CHECKLIST.md"
echo ""
echo "View the checklist:"
echo "  cat /root/vito/SETUP_CHECKLIST.md"
echo ""
echo "Or open it in an editor:"
echo "  nano /root/vito/SETUP_CHECKLIST.md"
